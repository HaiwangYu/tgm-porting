...
absolute drift ($x$) coordinate. A dedicated SCE simulation
map\footnote{The simulation map (the BNL ``SpaCE'' model) is a forward
simulation of the effect: a space-charge density taken linear in the drift
coordinate and independent of $y,z$ (reaching $90~\mathrm{nC/m^3}$ at the
cathode, set by the cosmic ionization rate and the $\sim 4~\mathrm{mm/s}$ ion
drift) is used to solve the Poisson problem for the field via a Fourier-series
solution, the field is interpolated everywhere with radial basis functions, and
ionization electrons are ray-traced through it (RKF45) using the $v(E)$ model to
predict the displaced reconstructed positions. This gives the forward
(true~$\rightarrow$~reco) map; the backward map is its inverse. It neglects
argon flow, non-uniform cosmic deposition, and the feedback of the field
distortion on the ion drift, which motivates the data-driven calibration.}
and a drift-velocity model $v(E)$ are also used. The method assumes cosmic muons
are \emph{straight} over the drift, the \emph{anode is undistorted} (ionization
there barely drifts, giving an absolute reference), and the SCE is
approximately \emph{time-stable} (tracks need not share an event). To keep the
straight-line assumption valid, only high-momentum tracks are used, with minimum
reconstructed (MCS) momenta that depend on the category
(Table~\ref{tab:sce-mcs}); tracks piercing both the anode and cathode are
rejected. The output is a set of \texttt{TH3F} maps on a $\approx 10$~cm voxel
grid, holding the backward (reconstructed~$\rightarrow$~true) offsets
$D_x,D_y,D_z$ and, after Section~\ref{ssec:sce-efield}, the relative field
distortions.
...
\begin{tabular}{lcc}
\hline
Track category & MC & Data \\
\hline
Anode-piercing & $\geq 3.3$ & $\geq 1.5$ \\
Cathode-piercing & $\geq 1.7$ & $\geq 1.1$ \\
Anode\,+\,cathode piercing & rejected & rejected \\
\hline
\end{tabular}
\caption{Minimum reconstructed (multiple-Coulomb-scattering) track momentum
[GeV] required per category in \texttt{CalibSCE}. Higher momenta give straighter
tracks; tracks that pierce both the anode and cathode are rejected in the
calibration call.}
\label{tab:sce-mcs}
\end{table}
...
\textbf{Algorithm.} Consider a track exiting the top face (normal $\hat{y}$).
Its true exit must lie on that face, whose position $y_{\rm top}$ is known, so
the \emph{face-orthogonal} component is obtained \emph{directly from data} as the
projected (perpendicular) distance of the reconstructed track end to the face,
\begin{equation}
\Delta y_{\rm reco}=y_{\rm top}-y_{\rm end}\quad(+\,4.5~\text{cm in data for the field-cage gap}),
\end{equation}
where $y_{\rm end}$ is the reconstructed end-point coordinate. The two
\emph{in-face} components cannot be measured from a single track, since the true
$x,z$ on the face are unknown and the reconstructed track is itself distorted.
They are recovered by assuming the true offset vector is \emph{parallel} to the
simulated one at that $(x,z)$, so all components share a common scale
$S(x,z)=\Delta y_{\rm reco}/\Delta y_{\rm sim}$ that is fixed by the measured
orthogonal component; hence
\begin{equation}
...
\end{equation}
In this way data sets the \emph{magnitude} through the directly-measured
orthogonal component (by far the largest), while the simulation supplies only the
\emph{direction} (the component ratios), which is robust to its approximate
space-charge density; this minimizes the dependence on simulation. The map is
built in 2D voxels ($\approx 10$~cm) per face. Because the gap blinds offsets
below $4.5$~cm, the near-anode region ($x<100$~cm) is filled from simulation,
scaled to the data-driven measurements at larger $x$. (For the
upstream/downstream faces $\Delta z$ is instead the orthogonal, data-driven
component.)
